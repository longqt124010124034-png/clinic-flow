...
CREATE TABLE public.device_configs (
  ...
  device_password_encrypted text, -- will be encrypted at app level
  connection_method text NOT NULL DEFAULT 'network', -- network, usb, api
  device_type text NOT NULL DEFAULT 'zkteco', -- zkteco, hikvision, other
  ...
  test_result text, -- json with error details if failed
...
CREATE TABLE public.device_sync_logs (
  ...
  sync_type text NOT NULL, -- initial, incremental, full_resync, manual
