...
CREATE TABLE public.attendance_records (
  ...
  -- Status
  attendance_status text NOT NULL DEFAULT 'present', -- present, absent, leave, holiday, sick, late, early_leave
...
CREATE POLICY "att read" ON public.attendance_records
  FOR SELECT TO authenticated USING (
    organization_id = public.current_org_id() OR employee_id IN (
      SELECT id FROM public.employees WHERE organization_id = public.current_org_id()
    )
  );
...
CREATE TABLE public.attendance_adjustments (
  ...
  adjustment_type text NOT NULL, -- manual_checkin, manual_checkout, late_excuse, leave_reason, overtime_assign
  reason text NOT NULL,
  adjusted_value text, -- time or duration value
...
-- ATTENDANCE SUMMARY (cached monthly data)
